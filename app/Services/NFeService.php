...
class NFeService
{
    ...
    private function buildProducts(Venda $venda)
    {
        ...
        foreach ($venda->items as $i => $item) {
            ...
            if (!$dadosFiscais) throw new \Exception("Dados fiscais não encontrados para o produto: {$produto->nome}");
            ...
            $std = new stdClass();
            $std->item = $i + 1; $std->vTotTrib = 0.00;
            ...
            $std = new stdClass();
            $std->item = $i + 1; $std->CST = $dadosFiscais->pis_cst;
            $std->vBC = 0; $std->pPIS = 0; $std->vPIS = 0;
            $this->nfe->tagPIS($std);

            $std = new stdClass();
            $std->item = $i + 1; $std->CST = $dadosFiscais->cofins_cst;
    ...
    private function buildPayments(Venda $venda)
    {
        ...
        if ($venda->pagamentos->isEmpty()) {
            ...
            foreach ($venda->pagamentos as $pagamento) {
                $det = new stdClass();
                $det->tPag = $pagamento->codigo_forma_pagamento; // Ex: 01-Dinheiro, 03-Cartão
        ...
        // A linha que criava a tag <pag> foi movida daqui para o início do método.
        // ===== FIM DA CORREÇÃO =====
    }
}