<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Reconciliação do Inventário #{{ $inventario->id }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Tabela de Divergências e Ação Final --}}
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                ...
                                @forelse ($itensComDiferenca as $item)
                                    <tr class="border-b">
                                        ...
                                        <td class="px-4 py-2 text-right font-bold {{ $item->diferenca > 0 ? 'text-green-500' : 'text-red-500' }}">
                                            R$ {{ number_format($item->diferenca * $item->produto->preco_custo, 2, ',', '.') }}
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4">Nenhuma divergência encontrada. O estoque contado confere com o sistema.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    @if($inventario->status !== 'finalizado')
                        <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <form method="POST" action="{{ route('inventarios.finalizar', $inventario) }}">
                                @csrf
                                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
                                    <div class="flex">
                                        <div class="py-1">
                                            <p class="text-sm text-yellow-700">
                                                <b>Atenção:</b> Esta ação é irreversível. Ao finalizar, o estoque dos produtos com divergência será permanentemente ajustado para a quantidade contada.
                                            </p>
                                        </div>
                                    </div>
                                </div>
