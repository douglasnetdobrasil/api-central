{{-- Este arquivo é para a tabela da página de listagem (index) E da Central de Faturamento --}}
    ...
    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
                ...
                {{-- NOVA COLUNA --}}
                <th scope="col" class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Valor Total</th>
        ...
        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            @forelse ($ordensServico as $os)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $os->equipamento ?? 'N/A' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{{ $os->data_entrada ? $os->data_entrada->format('d/m/Y H:i') : '-' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                            @if($os->status == 'Concluida' || $os->status == 'Concluída') bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                    ...
                    {{-- NOVO TD PARA O VALOR --}}
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-700 dark:text-gray-300 font-semibold">
                        R$ {{ number_format($os->valor_total, 2, ',', '.') }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {{-- Dropdown de Ações (O código aqui permanece o mesmo, com o @if que mostra 'Faturar Rápido' ou 'Ver Venda') --}}
                        <div x-data="{ open: false }" @click.away="open = false" class="relative inline-block text-left">
                            ...
                            <div x-show="open" x-transition ... class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none z-10" style="display: none;" role="menu">
                                <div class="py-1" role="none">
                                    <a href="{{ route('ordens-servico.edit', $os) }}" ... role="menuitem">
                                        <i class="fas fa-edit mr-2"></i> Editar OS
                                    </a>
                                    <a href="{{ route('ordens-servico.imprimir', $os) }}" target="_blank" ... role="menuitem">
                                        <i class="fas fa-print mr-2"></i> Imprimir OS
                                    </a>

                                    @if (($os->status == 'Concluida' || $os->status == 'Concluída') || $os->venda_id != null)
            ...
            @empty
                <tr>
                    {{-- Ajustar colspan para 7 colunas --}}
...
<div class="mt-4">
    {{-- Verifica se a variável é um Paginator antes de chamar links() --}}
    @if ($ordensServico instanceof \Illuminate\Pagination\LengthAwarePaginator)
        {{ $ordensServico->links() }}
    @endif
</div>