<div>
    <div x-data="{ tab: 'info' }">
        {{-- BOTÃO DE TESTE --}}
        <button wire:click="testar" class="m-4 p-2 bg-red-500 text-white font-bold">
            CLIQUE AQUI PARA TESTAR A COMUNICAÇÃO
        </button>
                        ...
                        <div x-show="$wire.get('frete_modalidade') != 9" class="p-4 border dark:border-gray-700 rounded-lg">
                            ...
                            @if ($transportadoraSelecionada)
                                <div class="mt-4 flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-700 rounded-md">
                                    <p class="font-semibold">{{ $transportadoraSelecionada->nome }}</p>
                                    <button wire:click="removerTransportadora" class="text-sm text-red-500">Remover</button>
                                </div>
                            @else
                                <div class="mt-4 relative">
                                    <x-text-input wire:model.live.debounce.300ms="transportadoraSearch" type="text" class="w-full" placeholder="Digite o nome ou CNPJ da transportadora..." />
                                    @if(!empty($transportadorasEncontradas))
                                        <ul class="absolute z-10 w-full mt-1 bg-white dark:bg-gray-900 border rounded-md shadow-lg">
                                            @foreach($transportadorasEncontradas as $t)
                                                <li wire:click="selecionarTransportadora({{ $t->id }})" class="px-4 py-2 cursor-pointer hover:bg-gray-100">{{ $t->nome }}</li>
                                            @endforeach
                                        </ul>
                                    @endif
                                </div>
                        ...
                        <div class="p-6 border dark:border-gray-700 rounded-lg">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                                <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
                                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Qtd. de Itens</span>
                                    <p class="text-2xl font-semibold dark:text-white">{{ count($cart) }}</p>
                                </div>
                                <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
                                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Qtd. de Produtos</span>
                                    <p class="text-2xl font-semibold dark:text-white">{{ $totalQuantidadeProdutos }}</p>
                                </div>
                                <div class="p-4 bg-gray-100 dark:bg-gray-700 rounded-lg">
                                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Peso Total</span>
                                    <p class="text-2xl font-semibold dark:text-white">{{ number_format($peso_bruto, 3, ',', '.') }} Kg</p>
                                </div>
                                <div class="p-4 bg-blue-100 dark:bg-blue-900 rounded-lg">
                                    <span class="text-sm font-medium text-blue-500 dark:text-blue-300">Valor Total da Nota</span>
                                    <p class="text-2xl font-bold text-blue-800 dark:text-blue-200">R$ {{ number_format($totalNota, 2, ',', '.') }}</p>
                                </div>
                            </div>
                            <div class="mt-8 border-t dark:border-gray-700 pt-6">
                                ...
                                <textarea wire:model.defer="informacoes_adicionais" rows="4" class="mt-2 block w-full rounded-md dark:bg-gray-900 border-gray-300 dark:border-gray-600" placeholder="Digite aqui informações complementares, dados de interesse do contribuinte, etc."></textarea>
                            </div>
                            <div class="mt-8 flex justify-end space-x-4">
                                <x-secondary-button wire:click="salvarRascunho">
                                    Salvar como Rascunho
                                </x-secondary-button>
                                <x-primary-button wire:click="emitirNFe" :disabled="!$clienteSelecionado || empty($cart)" class="disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div class="fixed inset-0 bg-gray-900 bg-opacity-60 z-40 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl p-6 m-4" @click.away="$wire.set('showPagamentoModal', false)">
            <h3 class="text-2xl font-bold dark:text-gray-100 mb-4">Pagamento</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 text-center">
                <div class="bg-gray-100 dark:bg-gray-700 p-3 rounded"><span class="text-sm block">Total</span><span class="text-xl font-bold">R$ {{number_format($total,2,',','.')}}</span></div>
                <div class="bg-blue-100 dark:bg-blue-800 p-3 rounded"><span class="text-sm block">Recebido</span><span class="text-xl font-bold">R$ {{number_format($valorRecebido,2,',','.')}}</span></div>
                <div class="bg-red-100 dark:bg-red-800 p-3 rounded"><span class="text-sm block">Falta</span><span class="text-xl font-bold">R$ {{number_format($faltaPagar,2,',','.')}}</span></div>
                <div class="bg-green-100 dark:bg-green-800 p-3 rounded"><span class="text-sm block">Troco</span><span class="text-xl font-bold">R$ {{number_format($troco,2,',','.')}}</span></div>
            </div>
            <div class="flex flex-wrap items-end gap-3 border-t dark:border-gray-700 pt-4">
                <div class="flex-grow"><x-input-label value="Forma de Pagamento" /><select wire:model="formaPagamentoSelecionada" class="mt-1 block w-full rounded-md dark:bg-gray-900">@forelse($formasPagamentoOpcoes as $f)<option value="{{$f->id}}">{{$f->nome}}</option>@empty<option disabled>Nenhuma</option>@endforelse</select></div>
                <div class="flex-grow"><x-input-label value="Valor" /><div class="relative"><x-text-input type="text" wire:model="valorPagamentoAtual" class="mt-1 w-full"/>@if($faltaPagar>0)<button wire:click="$set('valorPagamentoAtual','{{number_format($faltaPagar,2,'.','')}}')" type="button" class="absolute inset-y-0 right-0 px-3 text-sm text-blue-600">Restante</button>@endif</div></div>
                <x-secondary-button wire:click="adicionarPagamento" class="h-10">Adicionar</x-secondary-button>
            </div>
            <div class="mt-4 max-h-40 overflow-y-auto"><h4 class="text-sm font-medium mb-2">Pagamentos:</h4><ul class="space-y-2">@forelse($pagamentos as $i => $p)<li class="flex justify-between items-center bg-gray-50 dark:bg-gray-700 p-2 rounded"><span>{{$p['nome']}}: <span class="font-bold">R$ {{number_format($p['valor'],2,',','.')}}</span></span><button wire:click="removerPagamento({{$i}})" class="text-red-500 text-xs">Remover</button></li>@empty<li class="text-center text-gray-500 text-sm py-4">Nenhum pagamento.</li>@endforelse</ul></div>
            <div class="flex justify-between items-center mt-6 pt-4 border-t dark:border-gray-700">
                ...
                <div>@if(session()->has('error_modal'))<span class="text-red-500 text-sm mr-4">{{session('error_modal')}}</span>@endif <x-primary-button wire:click="salvarVendaParaNFe" :disabled="$faltaPagar > 0.009" class="text-lg disabled:opacity-50">Confirmar Venda</x-primary-button></div>
            </div>
        </div>
    </div>
    @endif
